...
## Purpose
This phase exists to profile the locally saved Twitter trending dataset and understand its structure before building normalization, topic matching, or NLP logic.

The local snapshot validation phase should already be complete.
This phase must use local files only.
...
- Work only from validated local snapshot artifacts
...
If the full local snapshot or validation report is missing, stop and report that the previous phase is incomplete.
...
This notebook should profile the local dataset only.
...
- the notebook exists
- the notebook runs on the validated local snapshot
