...
from snowflake_loader.snowpipe import load_pending_files_for_family as snowpipe_load_pending_files_for_family
...
def main(argv: list[str] | None = None) -> int:
    ...
    skip_completion_gate = bool(args.skip_completion_gate)
    ...
    gate_summary_payload: dict[str, object]

    if skip_completion_gate:
        state_db_path = config.state_db_path
        gate_summary_payload = {
            "skipped": True,
            "reason": "Completion gate bypassed via --skip-completion-gate for fixture/demo flow.",
        }
    else:
        ...
        gate_summary_payload = asdict(gate_summary)
            ...
            json.dumps(
                {
                    "mode": "dry_run",
                    "load_mode": load_mode,
                    "skip_completion_gate": skip_completion_gate,
                    "run_root": str(config.run_root),
                    "run_tag": config.run_tag,
                    "state_db_path": str(state_db_path) if state_db_path else None,
                    "completion_gate": gate_summary_payload,
                    "dataset_manifest": {
                        family: {
    ...
    per_family_snowpipe: dict[str, dict[str, int]] = {}
    ...
    with SnowflakeSession.connect_from_config(config) as session:
        for dataset_family in DATASET_FAMILIES:
            ...
            if load_mode == "copy":
                ...
                snowpipe_summary = snowpipe_load_pending_files_for_family(
                    session=session,
                    pending_files=pending_files,
                    dataset_family=dataset_family,
                    run_tag=config.run_tag,
                    load_invocation_id=config.load_invocation_id,
                    pipe_name=config.objects.pipe_names[dataset_family],
                    landing_table_name=config.objects.landing_table_names[dataset_family],
                    manifest_table_name=config.objects.manifest_table_name,
                )
                per_family_snowpipe[dataset_family] = {
                    "pipe_name": config.objects.pipe_names[dataset_family],
                    "stage_prefix": f"{config.run_tag}/{dataset_family}",
                    "attempted_files": snowpipe_summary.attempted_files,
                    "loaded_files": snowpipe_summary.loaded_files,
                    "failed_files": snowpipe_summary.failed_files,
                    "timed_out_files": snowpipe_summary.timed_out_files,
                    "loaded_rows": snowpipe_summary.loaded_rows,
                    "skipped_as_already_loaded": len(all_files) - len(pending_files),
                }
    ...
    if load_mode == "copy":
        payload["copy_summary"] = per_family_copy
    else:
        payload["snowpipe_summary"] = per_family_snowpipe
